...
public struct FDRectInset {
    // flag to ensure inset top is equal to inset bottom
    public static let npos: CGFloat = -1
    public static let identity: FDRectInset = FDRectInsetMake(0, 0, 0)
...
public func FDRectInsetMake(_ insetX: CGFloat, _ insetY: CGFloat, _ fixedWidth: CGFloat, _ fixedHeight: CGFloat = FDRectInset.npos) -> FDRectInset {
    return FDRectInset(insetX: insetX, insetY: insetY, fixedWidth: fixedWidth, fixedHeight: fixedHeight)
}

public struct FDPositionInset {
    public static let identity = FDPositionInsetMake(0, 0)
    
    public let insetLeft: CGFloat
    public let insetRight: CGFloat
}

public func FDPositionInsetMake(_ insetLeft: CGFloat, _ insetRight: CGFloat) -> FDPositionInset {
    return FDPositionInset(insetLeft: insetLeft, insetRight: insetRight)
}

public enum FDTextFiledStyle {
...
open class FDTextField: UITextField {
    ...
    open var leftViewInset: FDRectInset = .identity {
        didSet {
    ...
    open var rightViewInset: FDRectInset = .identity {
        didSet {
    ...
    open var separatorInset: FDPositionInset = .identity {
        didSet {
            assert(isSepatatorEnabled, "should enabled first")
            setNeedsLayout()
        }
    ...
    open var textAreaInset: FDPositionInset = .identity {
        didSet {
            setNeedsLayout()
        }
    }
    ...
    open override func leftViewRect(forBounds bounds: CGRect) -> CGRect {
        if leftView != nil {
            if leftViewInset.fixedHeight == FDRectInset.npos {
    ...
    open override func rightViewRect(forBounds bounds: CGRect) -> CGRect {
        if rightView != nil {
            if rightViewInset.fixedHeight == FDRectInset.npos {
    ...
    private let style: FDTextFiledStyle
    
    public init(_ style: FDTextFiledStyle = .custom) {
        self.style = style
